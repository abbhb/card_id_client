...
@app.route('/list_all_user_faces')
def list_all_user_faces():
    # 在每个请求处理函数中重新创建 FaceData 实例
    face_data = CardData(DB_NAME)
    user_faces = face_data.list_all_user_face()
    del face_data  # 在函数结束时关闭数据库连接
    return jsonify(user_faces)
...
def add_new_faces_in_image(file_stream, username, student_id, face_id):
    ...
    img_path_save = os.path.join(student_id,
                                 username + str(timestamp) + '.' + os.path.splitext(filename)[-1].strip("."))
    file_stream.save(
        os.path.join(img_path, username + str(timestamp) + '.' + os.path.splitext(filename)[-1].strip(".")))
...
def add():
    ...
    if request.method == 'POST':
        ...
        if student_id and username:
            face_data = CardData(DB_NAME)
            face_data.create_user(student_id, username)
            del face_data  # 在函数结束时关闭数据库连接
...
def delete():
    if request.method == 'POST':
        ...
        face_data = CardData(DB_NAME)  # 创建 FaceData 实例
