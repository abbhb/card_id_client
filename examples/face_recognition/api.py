...
def compare_faces(known_face_encodings, face_encoding_to_check, tolerance=0.6)-> (bool,int):
    """
    ...
    for shuju in face_distance(known_face_encodings, face_encoding_to_check):
        print(shuju)
        if shuju<=tolerance and shuju<=min:
            min = shuju
            weizhi = currentweizhi
            zhuangtai = True
        currentweizhi += 1
